...
assert ( accum == SAMPLE_ANSWER ) or  ( accum == PART1_ANSWER )
"""
...
def get_puzzle_one_string(puzzle):
    def sub():
        for prow in puzzle:
            if prow:
                for i in range(1,len(prow[0].data)-1):
                    yield  "".join(t.data[i][1:-1] for t in prow )
    return "\n".join(sub())                    
...
def later():
    for flip in [False,True]:
        for cc in range(4):
            ...
            yield get_puzzle_one_string( puzzle )            
